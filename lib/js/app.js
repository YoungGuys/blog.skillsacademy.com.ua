...
$(document).ready(function () {
    ...
    $(".carousel").each(function () {
        var count = $(this).find('.carousel__item').length;
        $(this)
            .find('.owl-next')
            .before(
                '<div class="owl-num">' +
                    '<span class="owl-num-current">1</span>' +
                    '/' + count +
                '</div>'
        );
    });
    ...
    function carouselCurrentSlide(_this) {
        _this = _this.parent().parent();

        _this
            .find('.owl-num-current')
            .text( _this.find('.owl-page.active').index() + 1 );
    }

    /*************************
     *************************/

    var slideText = $('.ex-slider__content__item');
    var slidePic = $('.ex-slider__pic__item');
    var slideCount = $('.ex-slider__content__item').length;

    slideText.eq(0).addClass('is-active');
    slidePic.eq(0).addClass('is-active');

    $('.js-btn_next_slide').click(function(){
        sliderControl();
    });

    $('.js-btn_prev_slide').click(function(){
        sliderControl('prev');
    });

    function sliderControl (type) {
        var currentSlide = slideText.filter('.is-active').index();
        ...
        if ( currentSlide >= slideCount) return false;

        slideText.removeClass('is-active');
        slidePic.removeClass('is-active');

        slideText.eq(currentSlide).addClass('is-active');
        slidePic.eq(currentSlide).addClass('is-active');
    };
